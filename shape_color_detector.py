...
def shape_color_detector (image): 
    ...
    # "~" is used when image have white back ground
    ...
    mask = np.zeros(i_img.shape[:2], dtype="uint8")

    for c in cnts:
        cv2.drawContours(mask, [c], -1, 255, -1)
        mask = cv2.erode(mask, None, iterations=2)
        mean = cv2.mean(cv2.cvtColor(i_img, cv2.COLOR_RGB2LAB), mask=mask)[:3]
    ...
    #cv2.imshow('window', np.hstack([i_img, cv2.cvtColor(thresh_img, cv2.COLOR_GRAY2BGR)]))
